...
"""
    ...
    def generate(self):
        ...
        # Remove any YAML that might have been generated by the LLM
        # Check if the content starts with ```yaml or ---
        if overview_content.startswith('```yaml'):
            # Remove the yaml code block
            parts = overview_content.split('```', 2)
            if len(parts) >= 3:
                overview_content = parts[2].strip()
        ...
        # Save overview WITHOUT YAML (will be added in step 5)
