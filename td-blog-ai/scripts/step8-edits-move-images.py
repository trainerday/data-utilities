#!/usr/bin/env python3
"""
Step 8: Finalize edits, extract images, and move to blog
1. Compare edited articles with originals
2. Extract edit instructions
3. Extract and track image URLs
4. Update workout-edits.json with image data
5. Move articles to blog system
"""
...
{{
  ...
  "custom_sections": [
    ...
    def finalize(self):
        ...
        print(f"\n🚀 Step 8: Finalizing edits, extracting images, and moving articles...")
        print("=" * 60)
        print(f"Query set: {self.query_set}")
        print("=" * 60)
        ...
        print(f"   ✏️  Articles with edits: {articles_with_edits}")
        print(f"   🖼️  Total images found: {total_images}")
        print(f"   📤 Articles copied to blog: {copied_count}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Step 8: Finalize edits, extract images, and move to blog')
